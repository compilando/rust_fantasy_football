...
use crate::models::{geometry::square::Square, race::Race, skill::Skill, team::Team};
...
impl FfPiece {
	...
	pub fn deserialize(&mut self, model: &str) {
		...
		let v: FfPiece = serde_json::from_str(&data).unwrap();
		self.title = v.title;
		self.cost = v.cost;
		self.ma = v.ma;
		self.st = v.st;
...
mod tests {
    ...
    #[test]
    fn serialize_works() {
        let piece = FfPiece::new();
        piece.serialize();        
    }

	#[test]
    fn build_human_works() {
        let mut piece = FfPiece::new();
